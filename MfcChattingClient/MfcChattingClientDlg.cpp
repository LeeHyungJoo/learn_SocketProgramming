...
// MfcChattingClientDlg.cpp : 구현 파일
//
...
void CMfcChattingClientDlg::OnBnClickedButtonConnect()
{
	...
	{
		AfxMessageBox(_T("ERROR: 소켓을 생성할 수 없습니다."));
		return;
	}

	if (m_Socket.Connect(_T("127.0.0.1"), 25000) == FALSE)
		AfxMessageBox(_T("ERROR: 서버에 연결할 수 없습니다."));
}
...
void CMfcChattingClientDlg::OnBnClickedButtonSend()
{
	...
	if (nLength > 0)
	{
		//사용자가 입력한 문자열(NULL문자 포함)을 서버에 전송한다.
...
{
	m_Socket.Close();
	AfxMessageBox(_T("연결을 종료하고 소켓을 닫았습니다."));
}
